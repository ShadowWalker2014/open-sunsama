...
- **AI native:** a hosted MCP server at https://api.opensunsama.com/mcp with OAuth sign-in. It works in Claude (claude.ai, Claude Desktop, Claude Code), ChatGPT, Cursor and any MCP client. 23 tools cover tasks, subtasks, time blocks (with estimate and actual minutes), the schedule for a day, and the user profile. There is also a public REST API with API keys.
- **Known gaps (say them when relevant, don't hide them):** the MCP server can't see synced calendar meetings yet, so tell readers to give the AI their meeting times or add their calendar's own connector (remove this line once calendar events ship over MCP). There are no built-in task integrations (Slack, Asana, Jira and so on), no Todoist or Sunsama importer, no auto-scheduler, and no guided planning ritual. The prebuilt desktop app can't point at a self-hosted server.
